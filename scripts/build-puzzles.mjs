 ...
 * (an actor appearing as themselves says nothing about their choices) — and
 * every film in SLATE itself, including ones this actor is billed in under a
 * different puzzle entry.
 *
 * That last exclusion matters more than it looks. The whole mechanic asks
 * "does this film beat or miss the baseline its cast implies?" — a question
 * that stops being fair the moment the film is allowed to grade its own
 * baseline. The leak is small for a 60-credit career (1/60) but real, and it
 * is not small for a thin one: an actor profiled from ten films has one of
 * them looking back at itself for every puzzle they appear in.
 */
async function profile(personId, name, profilePath) {
  const { cast = [] } = await tmdb(`/person/${personId}/movie_credits`);

  const ratings = cast
    .filter((film) => !slateMovieIds.has(film.id))
...
/**
 * Every SLATE film's TMDb id, resolved before any actor is profiled.
 *
 * Has to happen up front rather than as the main loop goes: an actor can be
 * cast in a film that appears later in SLATE than the one that first
 * introduces them (Al Pacino is profiled while processing Heat, but Righteous
 * Kill, Jack and Jill and Gigli — all also in SLATE — haven't been resolved
 * yet at that point). Without this pre-pass, the exclusion above would only
 * ever catch the film an actor happened to be *first* encountered in.
 */
const slateMovieIds = new Set();
for (const entry of SLATE) {
  const search = await tmdb('/search/movie', { query: entry.title, year: entry.year });
  if (search.results?.[0]) slateMovieIds.add(search.results[0].id);
}
console.error(`resolved ${slateMovieIds.size}/${SLATE.length} slate films for self-reference exclusion`);
 ...
 * (films with at least ${MIN_VOTES} votes, documentaries and SLATE's own
 * films excluded — no actor's baseline is computed from the film it is judging).
...
// Real API data, not a guess — the UI stops labelling every number "estimated".
export const DATA_STATUS = 'verified';

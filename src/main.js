...
els.puzzleNo.title ||= 'Tap three times to play any film';
if (override) {
...
for (const modal of [els.help, els.stats, els.films]) {
  modal.addEventListener('click', (e) => {
    if (e.target === modal || e.target.hasAttribute('data-close')) closeModal(modal);
  });
}
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') [els.help, els.stats, els.films].forEach(closeModal);
});
...
/**
 * Lets any film in the slate be played on demand.
 *
 * Reached by tapping the puzzle number three times rather than by a URL: the
 * page may be embedded in a viewer that never passes a query string through, so
 * a URL-only route is one that silently does not work.
 */
function renderFilmPicker() {
  els.filmList.replaceChildren();
  for (const film of slate()) {
    const btn = document.createElement('button');
    btn.type = 'button';
    btn.textContent = `${film.title} (${film.year})`;
    if (film.id === puzzle.id) btn.classList.add('is-current');
    btn.addEventListener('click', () => playFilm(film.id));
    els.filmList.append(btn);
  }
}

let taps = 0;
let tapTimer = null;
els.puzzleNo.addEventListener('click', () => {
  taps += 1;
